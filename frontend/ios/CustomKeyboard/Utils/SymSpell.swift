...
public class SymSpell {
    ...
    private let buildDeletesIndex: Bool

    public init(maxDictionaryEditDistance: Int = 2, prefixLength: Int = 7, buildDeletesIndex: Bool = true) {
        ...
        self.buildDeletesIndex = buildDeletesIndex
    }
    ...
    /// Free internal memory held by SymSpell dictionaries.
    public func dispose() {
        deletes.removeAll(keepingCapacity: false)
    ...
    public func wordSegmentation(_ input: String, maxEditDistance: Int = 0) -> Segmentation {
        ...
        for j in 0 ..< input.count {
            ...
            for i in 1 ... min(input.count - j, maxDictionaryWordLength) {
                let start = input.index(input.startIndex, offsetBy: j)
                let end = input.index(start, offsetBy: i)
                var partSubstr: Substring = input[start..<end]
    ...
    /// - Returns: Array of `SuggestItem`  representing suggested correct spellings for the input string.
    public func complete(_ input: String, isEnglish: Bool) -> [SuggestItem] {
        let uncodeInput = input.unicodeScalars
        var uncodeInput2 = input
        uncodeInput2.removeAll(where: { $0 == "\n"})
        let results = words.values.filter {
        if isEnglish{
                return $0.0.unicodeScalars.starts(with: uncodeInput)
            }

        return $0.0.chars.starts(with: uncodeInput2.chars)
        }
//        Logger.debug("results=\(results.count) uncodeInput2=\(uncodeInput2)")
    ...
    private func edits(_ word: Substring, _ editDistance: Int, _ deleteWords: inout Set<Substring>) {
        ...
        // Match suggestion casing to the user's original typed prefix
...
// MARK: - String Damerau-Levenshtein Distance

extension String {
    /// Computes the Damerau-Levenshtein distance between the receiver and another string.
    /// Implements the Optimal String Alignment variant with transpositions.
    func distanceDamerauLevenshtein(between other: String) -> Int {
        let s = Array(self)
        let t = Array(other)
        let n = s.count
        let m = t.count
...
final class SymSpellProvider {
    static let shared = SymSpellProvider()

    // Cache SymSpell instances per language (e.g., "en", "fa")
    private var symSpells: [String: SymSpell] = [:]
    private var loadingLanguages: [String: Bool] = [:]

    private init() {}

    // Load dictionary once per language and return shared instance
    func getSymSpell(for localeIdentifier: String) async -> SymSpell? {
        // Resolve a stable language key (e.g., "en", "fa")
        let languageKey = Locale(identifier: localeIdentifier).languageCode ?? localeIdentifier

        if let instance = symSpells[languageKey] { return instance }
        if loadingLanguages[languageKey] == true {
            return nil
        }

        loadingLanguages[languageKey] = true
        let sym = SymSpell(maxDictionaryEditDistance: 2, prefixLength: 7, buildDeletesIndex: false)

        // Map language to dictionary resource
        let resourceName: String
        switch languageKey {
        case "fa":
            resourceName = "wiki_fa_80k" // Persian dictionary
        default:
            resourceName = "frequency_dictionary_en_82_765" // English dictionary
        }

        if let url = Bundle(for: KeyboardViewController.self).url(forResource: resourceName, withExtension: "txt") {
            do {
                try await sym.loadDictionary(from: url, termIndex: 0, countIndex: 1)
                Logger.debug("SymSpell dictionary loaded for \(languageKey): \(sym)")
                self.symSpells[languageKey] = sym
            } catch {
                Logger.debug("SymSpell dictionary load failed for \(languageKey): \(error)")
                self.symSpells[languageKey] = nil
            }
        } else {
            Logger.debug("SymSpell dictionary file not found in bundle for \(languageKey): \(resourceName).txt")
            self.symSpells[languageKey] = nil
        }

        loadingLanguages[languageKey] = false
        return symSpells[languageKey]
    }

    // Backwards compatibility: default to English
    func getSymSpell() async -> SymSpell? {
        return await getSymSpell(for: "en")
    }

    // Release shared instances and free memory
    func unload(languageIdentifier: String? = nil) {
       loadingLanguages.removeAll()

        if let lang = languageIdentifier {
            let key = Locale(identifier: lang).languageCode ?? lang
            symSpells[key]?.dispose()
            symSpells[key] = nil
        } else {
            for (key, sym) in symSpells { sym.dispose(); symSpells[key] = nil }
        }
    }
}
