...
- **Name new `UniqueConstraint` / `CheckConstraint` / `Index` explicitly.** The
  project deliberately does **not** impose a global `MetaData` naming
  convention, because the live database was built by `create_all` with
  PostgreSQL's default constraint names; introducing a divergent convention now
  would make future `ALTER`/`DROP` ops fail against production. Until a planned
  one-time rename aligns existing names, give every *new* named constraint an
  explicit `name=` so it can be altered later.
