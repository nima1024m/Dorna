...
@pytest.mark.anyio
async def test_get_onboarding_unset_returns_defaults(client):
    fake_session = _FakeSession(
        results=[
            _FakeResult(scalar=None),          # UserPreferences
    ...
    try:
        res = await client.get("/v1/user/onboarding/")
    finally:
        _reset_overrides()
...
@pytest.mark.anyio
async def test_put_onboarding_first_time_save(client):
    ...
    _override_deps(fake_session)
    try:
        res = await client.put("/v1/user/onboarding/", json=payload)
    finally:
        _reset_overrides()
...
@pytest.mark.anyio
async def test_put_onboarding_updates_existing(client):
...
@pytest.mark.anyio
async def test_put_onboarding_invalid_category_ids_returns_400(client):
    fake_session = _FakeSession(
        results=[
...
@pytest.mark.anyio
async def test_put_onboarding_invalid_goal_ids_returns_400(client):
    fake_session = _FakeSession(
        results=[
            _FakeResult(scalars_list=["internet"]),  # validate categories
...
@pytest.mark.anyio
async def test_put_onboarding_rejects_empty_lists(client):
...
@pytest.mark.anyio
async def test_get_onboarding_categories_and_goals(client):
    fake_session = _FakeSession(
        results=[
    ...
    _override_deps(fake_session)
    try:
        res1 = await client.get("/v1/user/onboarding/categories")
        res2 = await client.get("/v1/user/onboarding/goals")
    finally:
        _reset_overrides()
